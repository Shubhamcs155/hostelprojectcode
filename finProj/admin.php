        ...
        return;
            ...
            xmlhttp.onreadystatechange = function() {
                if (this.readyState == 4 && this.status == 200) {
                    ...
                    var c = "b".concat(p.slice(0,p.length - 1));
                    let k = document.getElementById(c);
                    k.style.display = "none";
                }
            };
            xmlhttp.open("GET","adminapar.php?hid="+st,true);
            xmlhttp.send();
        }
    }
</script>
        ...
        if($result)
        {
            if(mysqli_num_rows($result)>=1)
            {
               while ($row= mysqli_fetch_assoc($result)) {
                   ...
                   <td class="aapr"><button class="btn btn-primary btn-sm bapr" value="'.$row['id'].'a" onclick="t(this.value)">Approve</button></td>
                   <td class="arej"><button class="btn btn-primary btn-sm brej" value="'.$row['id'].'r" onclick="t(this.value)">Reject</button></td>
               </tr>';
               $k = $k + 1;
               }
            }
