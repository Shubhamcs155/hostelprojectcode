<?php
    ...
    if (isset($_SESSION['admin'])) {
        $msg = "Admin Can't Apply for any hostel from this account";
        echo $msg;
    }
    else if (isset($_GET['hid']) && isset($_SESSION['user'])) 
    {
        $uid = $_SESSION['user'];
        $hid = intval($_GET['hid']);
        ...
        if (mysqli_num_rows($result)==0) 
        {
            ...
            if($result) {
                if (mysqli_num_rows($result)==0) {
                    ...
                    $result1 = mysqli_query($conn,$query);
                    // if($result){echo "successful";}else {echo "sub dddddddddddddd failed";}
                }
